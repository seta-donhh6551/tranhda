...
<div id="wrap-content" class="wrap-bg-white">
  ...
  <div id="bg_type_line">
    ...
    <div id="cat-content">
      ...
      <div id="news">
      	<?php
		foreach($listalls as $list){
			echo "<div class='list_news'>";
			if($list['con_image'] != NULL){
				echo "<img src='".base_url()."uploads/news/$list[con_image]' class='news_img' width='120' align='left' style='margin:10px 10px 10px 0px' />";
			}
			echo "<h3 class='news_h3'><a href='".base_url()."nghe-thuat-tranh-gao/$list[con_rewrite]-$list[con_id].html' title='".$list['con_title']."' style='font-size:14px;color:#ababab'>".$list['con_title']."</a></h3>";
			//echo "<div class='news_date'><i>".$list['news_author']." - ".$list['news_date']."</i></div>";
			echo "<div class='news_info'>".$list['con_info']."</i></div>";
			echo "</div>";
